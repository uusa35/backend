@if(isset($categories) && $categories->isNotEmpty())
    @if($categories->isNotEmpty())
        @if(env('APP_CASE') === 'EVENTCOM')
            ...
            @endforeach
        @else
            @foreach($categories->where('is_user',true)->where('is_parent', true)->where('on_home', true)->take(env('ENABLE_NAV_CATEGORY') ? 4 : 6) as $cat)
                <li class="dropdown megamenu">
                    <a href="{{ route('frontend.user.search',['user_category_id' => $cat->id]) }}">{{ str_limit($cat->name,15,'') }}</a>
                                ...
                                <div class="col-sm-9">
                                    <div class="row tt-col-list">
                                        @foreach($cat->children->where('on_home', true) as $sub)
                                            <div class="col-sm-4">
                                                <a href="{{ route('frontend.user.search',['user_category_id' => $sub->id]) }}"
                                                   class="tt-title-submenu">
                                                    {{ $sub->name }}
                                                    @if($sub->image)
                                                        <img class="img-menu-category img-responsive"
