    ...
    <div class="tt-table-responsive">
        <table class="tt-table-shop-01">
            ...
            @if(!env("ISTORES"))
                ...
                @endif
                @if($element->user->country && !env('HOMEKEY') && !env('ISTORES'))
                    <tr>
                        <td class="td-fixed-element">
                ...
                @if(!is_null($element->individuals))
                    <tr>
                        <td class="td-fixed-element">
                            <span><i
                                    class="fa fa-fw fa-lg icon-f-95"></i> {{ trans('general.individuals_count') }}: </span>
                        </td>
                        <td>
                            {{ $element->individuals }}
                        </td>
                ...
                @if($element->is_available && !env('DAILY') && !env('HOMEKEY'))
                    <tr>
                        <td class="td-fixed-element">
                            <span><i
                                    class="fa fa-fw fa-lg icon-e-74"></i> {{ trans('general.available_items') }}:</span>
                        </td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($element->tags as $tag)
                                <a class="theme-color"
                                   href="{{ route('frontend.product.search',['tag_id' => $tag->id]) }}">
                                    {{ $tag->slug }},
                                </a>
                            @endforeach
                        </td>
                ...
                @if(!is_null($element->brands) && $element->brands->isNotEmpty())
                    <tr>
                        <td class="td-fixed-element"><i
                                class="icon-f-09 fa fa-fw fa-lg"></i>{{ trans("general.brands") }}:
                        </td>
                        <td>
