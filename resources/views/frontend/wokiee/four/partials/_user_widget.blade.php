    ...
    <div class="tt-description">
        <div class="tt-row">
            <ul class="tt-add-info">
                <li>
                    <a href="{{ route('frontend.user.show.name', ['id' => $element->id, 'name' => $element->name]) }}">{{ $element->slug }}</a>
                </li>
            </ul>
            {{--@include('frontend.wokiee.four.partials._rating')--}}
        </div>
        <h2 class="tt-title">
            <a href="{{ route('frontend.user.show.name',['id' => $element->id , 'name' => $element->name]) }}">{{ str_limit($element->description,100,'...') }}</a>
        </h2>
