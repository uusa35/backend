                    ...
                    <table id="dataTable" class="table table-striped table-bordered table-hover" cellspacing="0">
                        ...
                        @foreach($element->roles as $role)
                            <tr>
                                ...
                                <td>
                                    <span class="label {{ activeLabel($role->pivot->index) }}">{{ activeText($role->pivot->index) }}</span>
                                </td>
                                <td>
                                    <span class="label {{ activeLabel($role->pivot->view) }}">{{ activeText($role->pivot->view) }}</span>
                                </td>
                                <td>
                                    <span class="label {{ activeLabel($role->pivot->create) }}">{{ activeText($role->pivot->create) }}</span>
                                </td>
                                <td>
                                    <span class="label {{ activeLabel($role->pivot->update) }}">{{ activeText($role->pivot->update) }}</span>
                                </td>
                                <td>
                                    <span class="label {{ activeLabel($role->pivot->delete) }}">{{ activeText($role->pivot->delete) }}</span>
                                </td>
                                <td>
                                    <div class="btn-group">
