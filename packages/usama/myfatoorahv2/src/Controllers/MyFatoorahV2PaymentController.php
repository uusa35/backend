...
//$apiKey = ''; //Live token value to be placed here: https://myfatoorah.readme.io/docs/live-token
    ...
    public function makePayment(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                return redirect()->home()->withErrors($validator->errors()->first());
            }
            /* ------------------------ Configurations ---------------------------------- */
//Test
...
//            return $data['invoiceURL'];
    ...
    public function error(Request $request)
    {
        ...
        try {
            ...
            Mail::to($settings->email)->send(new OrderFailed($order, $settings, trans('general.process_failure')));
            $markdown = new Markdown(view(), config('mail.markdown'));
            return $markdown->render('emails.orders.failed', ['order' => $order]);

        } catch (\Exception $e) {
            Mail::to($settings->email)->cc($order->user->email)->send(new OrderFailed($order, $settings, $e->getMessage()));
